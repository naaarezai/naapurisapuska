...
import 'ai_service_web.dart'; // for AIServiceImplementation interface

/// Mobile implementation using ML Kit
class AIServiceImpl implements AIServiceImplementation {
  ...
  Future<FoodRecognitionResult?> recognizeFood(XFile imageFile) async {
    try {
      if (kDebugMode) {
        print(
            '🔍 Aloitetaan kuvan tunnistus ML Kit:llä (ilmainen, offline)...');
      }

      // 1. Lataa kuva (Convert XFile to File for ML Kit)
      ...
      if (foodLabel == null && labels.isNotEmpty) {
        ...
        if (kDebugMode) {
          print(
              'ℹ️ Ruokaa ei löytynyt, käytetään ensimmäistä labelia: $foodLabel');
        }
      }
