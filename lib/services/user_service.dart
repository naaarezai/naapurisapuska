...
class UserService {
  ...
  Future<String?> uploadProfileImage(XFile imageFile, String userId) async {
    try {
      ...
      // HUOM: Polun on oltava 'profile_images/$userId/...' jotta storage.rules toimii
  ...
  /// Kasvata käyttäjän jaettujen annosten määrää
  Future<void> incrementSharedCount(String userId) async {
    try {
      await _firestore.collection('users').doc(userId).update({
        'totalShared': FieldValue.increment(1),
      });
      if (kDebugMode) {
        print('✅ Käyttäjän totalShared kasvatettu');
      }
    } catch (e) {
      if (kDebugMode) {
        print('❌ totalShared:n päivitys epäonnistui: $e');
      }
      rethrow;
    }
  }

  /// Vaihda ruoan suosikki-status (lisää/poista)
  Future<void> toggleFavorite(String userId, String foodId) async {
    try {
      ...
      final isFavorite = favorites.contains(foodId);
      ...
      if (kDebugMode) {
        print('❌ Suosikin vaihto epäonnistui: $e');
      }
      rethrow;
    }
  }
  ...
  Future<void> deleteAccount() async {
    try {
      ...
      try {
        // Yritä poistaa vanha polku (yhteensopivuus)
